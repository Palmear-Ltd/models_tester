...
**Two new additive features (2026-08-09/12), both born from a corpus-wide investigation into whether RMS energy and click-transient timing/spectrum (already computed live but never fed to the model) could help — see specs/plans `docs/superpowers/{specs,plans}/2026-08-09-rms-verdict-confidence*.md` and `2026-08-09-rootcause-click-template-refinement*.md` for the full evidence trail (several hypotheses tested and discarded — individual click shape, literature-derived frequency-band matching — before landing on what actually held up under held-out testing):**

- **RMS-based verdict confidence** (`app/decision/rms_confidence.py`, `models/9_1_2/rms_confidence.json`): a second, independent confidence number surfaced next to the session verdict — *"INFESTED (EWMA peak: 0.71) — verdict confidence: low"* — built from two logistic curves (one per verdict branch) mapping session RMS to P(verdict correct), fit via `evaluate_decision_rules.py --rms-manifest/--rms-confidence-out` on the same train/test split as the decision cutoff. Session RMS is a weak *direct* predictor of infestation, but a much stronger predictor of *which way the model's verdict is wrong* — false positives run hot, false negatives run quiet. Purely additive: never changes `state`/`predicted_infested`, mirrors `anomaly_result`'s "confidence only, never state" pattern. `scripts/rms_scan.py` is the permanent tool for backfilling RMS onto a scored manifest.
- **T010 `ClickSpectralMatchCheck`** (`app/health/checks/time_domain.py`, `app/health/click_template.json`): refines `rootcause.py`'s SENSOR_LINK attribution. T009's raw click *count* can't tell a real bite click from sensor/cable contact-noise (confirmed-fault sessions click *more* than genuinely infested ones). T010 scores each click's spectrum against a locally-fit differential template (real-infestation click spectra minus clean-baseline click spectra — literature frequency constants were tried first and failed to separate fault from real signal; a template fit against *this project's own* corpus did) and judges a window by `match_fraction`, not raw count. Fit via the new root-level `fit_click_template.py` (mirrors `calibrate.py`'s split — WAV I/O stays out of `app/health/`). Wired into `rootcause.py`'s `_WEIGHT_TABLE` (`MAX_SCORE` 10→12) with a refit `DEFAULT_SESSION_CUTOFF` (0.25→0.75). **Explicitly scoped**: validated against the *confirmed* SENSOR_LINK fault corpus (AUC ~0.75–0.85), not a general false-positive fix — a broader sample of model-flagged false positives showed only marginal improvement, since not every false positive is a sensor-link fault. Treat a T010-driven SENSOR_LINK verdict with that narrow reading, per the weight-table comment.

Both were built by two agents working the fully-written specs/plans end to end (`docs/superpowers/`), each independently verified (test suite, headless import, spot-checked diffs) before being accepted — 349 tests pass (up from 259).
